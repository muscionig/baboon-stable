# -*- coding: utf-8 -*-
"""
Created on Wed May 16 18:27:54 2018
...
day3Files = glob.glob('../data/group/0803/*.csv')
day3 = readDataset(day3Files)


day4Files = glob.glob('../data/group/0804/*.csv')
day4 = readDataset(day4Files)

trainA = day3.append(day4, ignore_index=True)

#%%
train = trainA.copy()
label = train.pop('LABEL_GROUP')
...
for f in range(0,10):  
    trainC = trainA.copy()
    mapping = dict(enumerate(np.unique(trainC.LABEL_GROUP)))
    print("Remapping label for XGB:")
    mapping = {v: k for k, v in mapping.items()}
    print(mapping)
    trainC.LABEL_GROUP = trainC.LABEL_GROUP.map(mapping)
    label = trainC.pop('LABEL_GROUP')
...
resultDataframe.to_csv('group_result_'+ str(current_milli_time())+'.csv', index = False)
